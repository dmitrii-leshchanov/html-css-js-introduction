...
// DOM elements
const allMoviesList = document.querySelector(".movies-list");
const sectionElements = document.querySelectorAll("section");
const popularMovie = document.querySelector(".popular-movies-list");
const nonPopularMovie = document.querySelector(".nonpopular-movies-list");
allMoviesList.innerHTML = getMoviesInfo();
popularMovie.innerHTML = getPopularMovieInfo();
nonPopularMovie.innerHTML = getNonpopularMovieInfo();

// Functions
function getMoviesInfo() {
    const arMovieInfo = moviesData.results.map(movie => 
        `<li class="movie-item">
    ...
    return arMovieInfo.join('');
}

function getPopularMovieInfo() {
    const sortedByPopularity = moviesData.results.sort((e1, e2) => e2.popularity - e1.popularity);
    const popularMovieInfo =  
        `<li class="movie-item">
            <img class="movie-poster" src="${httpPrefix}${sortedByPopularity[0].poster_path}">
            <div class="movie-details">
                <h1 class="movie-title">${sortedByPopularity[0].original_title}</h1>
                <p class="movie-descripton">${sortedByPopularity[0].overview}<p>
            </div>
        </li>`;
    return popularMovieInfo;
}

function getNonpopularMovieInfo() {
    const sortedByPopularity = moviesData.results.sort((e1, e2) => e1.popularity - e2.popularity);
    const popularMovieInfo =  
        `<li class="movie-item">
            <img class="movie-poster" src="${httpPrefix}${sortedByPopularity[0].poster_path}">
            <div class="movie-details">
